        ...
        <div className="p-6 sm:p-7">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {project.stack.map((item) => (
              <span
                key={item}
                className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-sm font-medium text-slate-100"
              >
                <span className="flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-slate-950/70 text-cyan-200">
                  <SkillIcon skill={item} className="h-4 w-4" />
                </span>
                {item}
              </span>
            ))}
          ...
          <div className="mt-8 flex flex-wrap gap-5 text-sm font-semibold">
            {project.links.map((link) => (
              <a
                ...
                className="hover-link relative inline-flex items-center text-slate-100"
              >
                {link.label}
              </a>
            ))}
